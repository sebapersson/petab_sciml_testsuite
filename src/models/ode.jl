function get_odeproblem(ode_id::Symbol, nn_models::Dict)
    ...
    if ode_id == :UDE5
        lv! = let nn = nn_models
            (du, u, p, t) -> lv_ude5!(du, u, p, t, nn)
        end
        p_mechanistic = (alpha = 1.3, delta = 1.8, beta = 0.9, x1 = 1.0, x2 = 2.0, x3 = 3.0)
        pnn = Lux.initialparameters(rng, nn_models[:net6][2])
        p_ode = ComponentArray(merge(p_mechanistic, (net6 = pnn,)))
        u0 = [0.44249296, 4.6280594]
        return ODEProblem(lv!, u0, (0.0, 10.0), p_ode)
    end
end
...
function lv_ude5!(du, u, p, t, nn_models)
    prey, predator = u
    @unpack alpha, delta, beta, x1, x2, x3 = p

    st6, nn6 = nn_models[:net6]
    du_nn = nn6([prey, x1, x2, x3], p.net6, st6)[1][1]

    du[1] = alpha * prey - beta * prey * predator # prey
    du[2] = du_nn * prey * predator - delta * predator # predator
    return nothing
end
