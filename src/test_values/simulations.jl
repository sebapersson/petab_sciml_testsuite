function save_simulations(
        x, llh_id::Symbol, ode_problem::ODEProblem, nn_models,
...
function get_simulations(
    ...
    if llh_id in [:UDE1, :UDE2, :UDE3]
        ...
        sol = solve(
            _ode_problem, Vern9(), abstol = 1.0e-12, reltol = 1.0e-12,
    ...
    if llh_id == :pre_ODE2
        ...
        sol1 = solve(
            ode_problem1, Vern9(), abstol = 1.0e-12, reltol = 1.0e-12,
            saveat = unique(measurements.time)
        )
        sol2 = solve(
            ode_problem2, Vern9(), abstol = 1.0e-12, reltol = 1.0e-12,
            saveat = unique(measurements.time)
        )
    ...
    if llh_id == :pre_ODE9
        ...
        _ode_problem = remake(ode_problem, p = p)
        sol = solve(
            _ode_problem, Vern9(), abstol = 1.0e-12,
            reltol = 1.0e-12, saveat = unique(measurements.time)
        )
        simulated_values = vcat(sol[1, :], sol[2, :])
    end
