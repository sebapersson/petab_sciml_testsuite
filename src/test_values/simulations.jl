function save_simulations(x, llh_id::Symbol, ode_problem::ODEProblem, nn_models,
        measurements::DataFrame, inputs, dir_save)
...
function get_simulations(x, llh_id::Symbol, ode_problem::ODEProblem, nn_models,
    ...
    if llh_id == :pre_ODE2
        st, nn_model = nn_models[:net1]
        p1 = (alpha = 1.3, delta = 1.8, beta = 0.9,
            gamma = nn_model([10.0, 20.0], x.net1, st)[1][1])
        p2 = (alpha = 1.3, delta = 1.8, beta = 0.9,
            gamma = nn_model([1.0, 1.0], x.net1, st)[1][1])
        ode_problem1 = remake(ode_problem, p = p1)
        ode_problem2 = remake(ode_problem, p = p2)
        sol1 = solve(ode_problem1, Vern9(), abstol = 1e-12, reltol = 1e-12,
            saveat = unique(measurements.time))
        sol2 = solve(ode_problem2, Vern9(), abstol = 1e-12, reltol = 1e-12,
            saveat = unique(measurements.time))
    ...
    if llh_id == :pre_ODE8
        st, nn_model = nn_models[:net3]
        p1 = (alpha = 1.3, delta = 1.8, beta = 0.9,
            gamma = nn_model(inputs[1], x.net3, st)[1][1])
        p2 = (alpha = 1.3, delta = 1.8, beta = 0.9,
            gamma = nn_model(inputs[2], x.net3, st)[1][1])
        ode_problem1 = remake(ode_problem, p = p1)
        ode_problem2 = remake(ode_problem, p = p2)
        sol1 = solve(ode_problem1, Vern9(), abstol = 1e-12, reltol = 1e-12,
            saveat = unique(measurements.time))
