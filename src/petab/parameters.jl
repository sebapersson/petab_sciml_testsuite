...
function save_parameters_table(petab_parameters_ids::Vector{Symbol}, nets_info::Dict, estimate_net_parameters::Bool, dir_petab)
    df_mech = DataFrame()
    ...
    df_nn = DataFrame(parameterId = net_ids, parameterScale = :lin, lowerBound = "-inf", upperBound = "inf", nominalValue = net_files, estimate = Int(estimate_net_parameters))
