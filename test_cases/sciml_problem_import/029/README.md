...
Test case for when a feed-forward neural network is a part of the ODE right-hand side, and the neural network has two
input arguments. Basically test case 001, but the input is split over two input arguments.
...
The inputs to the network are the `prey` and `predator` species, which are each provided as separate input arguments.
