...
nn_model = @compact(layer1=ConvTranspose((5, 5), 1 => 2; cross_correlation = true),
    layer2=ConvTranspose((2, 5), 2 => 1; cross_correlation = true)) do x
    embed = layer1(x)
    out = layer2(embed)
    @return out
end
...
for i in 1:3
    ...
    input = rand(rng, Float32, 15, 10, 1, 1)
